 ...
 * 
 * code added by: Vincent Li    vinli@bu.educa
 */
...
public class LetterSquare {
    ...
    private boolean isValid(String letter, int wordNum, int charNum) {        
        /* Replace this line with your implementation of the method. */
        // first word first letter
        if (this.words[0].equals("")) {
            return true;
        }
        // first letter of following word
        if (wordNum > 0 && charNum == 0 && lastLetter(this.words[wordNum - 1]).equals(letter)) {
            return true;
        }
        // rest of the word
        if (charNum > 0 && !this.onSameSide(lastLetter(this.words[wordNum]), letter) 
            && dictionary.hasString(this.words[wordNum] + letter) 
            && !this.alreadyUsed(this.words[wordNum] + letter)) {
            return true;
        }
    ...
    private boolean solveRB(int wordNum, int charNum, int maxWords) {
        // base case 1: wasn't able to find the solution within constraint
        // of the maximum amount of words
        if (wordNum >= maxWords) {
            return false;
        }

        // base case 2: puzzle is solved
        ...
        for (String letter : this.letters) {
            ...
            if (this.isValid(letter, wordNum, charNum)) {
                ...
                // no more letters can be added, and is a word that fulfills solution
                if (dictionary.hasFullWord(this.words[wordNum]) && this.words[wordNum].length() >= 3) {   
                    // move onto building the next word
